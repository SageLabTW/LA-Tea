...
這樣的圖分割除了給出一個合理的近似解以外，它還有一個附帶的好處。捷克數學家[費德勒](https://en.wikipedia.org/wiki/Miroslav_Fiedler)（Miroslav Fiedler，1926--2015）[^fiedler] 證明了這樣的分割會讓區塊連通。
...
2. 將 $$10$$ 個點的路徑圖 $$P_{10}$$ 利用第 $$2$$ 位拉普拉斯特徵向量做圖分割，並比較此分割與圖嵌入的結果。
3. 將 $$10$$ 個點的圈圖 $$C_{10}$$ 利用第 $$2$$ 位拉普拉斯特徵向量做圖分割，並比較此分割與圖嵌入的結果。
4. 找一個例子讓 $$G[N_{< 0}]$$ 是不連通的。
5. 若 $$G$$ 是賦權圖，說明 $$\frac{1}{4}\by\trans L\by$$ 代表的是什麼，以及我們該如何定義 $$\vert{}E(A,B)\vert{}$$。
